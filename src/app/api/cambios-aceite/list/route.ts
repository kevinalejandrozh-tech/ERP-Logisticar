...
export async function GET() {
  try {
    await ensureSchema();
    const pool = getPool();
    const result = await pool.query(`SELECT id, eco, unidad, fecha_ultimo_cambio, km_ultimo_cambio, km_actual, servicio_realizado FROM cambios_aceite ORDER BY id DESC`);
    const registros = result.rows.map((r) => ({
      ...
      servicioRealizado: !!r.servicio_realizado,
    }));
