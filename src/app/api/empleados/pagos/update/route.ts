...
const CAMPOS: Record<string, string> = {
  ...
  totalEntregado: "total_entregado",
  metodoPago: "metodo_pago",
  banco: "banco",
  ultimos4Cuenta: "ultimos_4_cuenta",
...
export async function POST(req: NextRequest) {
  try {
    ...
    await ensureSchema();
    const pool = getPool();
    await pool.query(`UPDATE empleados_pagos SET ${CAMPOS[campo]} = $2, updated_at = now() WHERE id = $1`, [id, body[campo] === "" ? null : body[campo]]);
