...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await pool.query(
      soloTms
        ? `SELECT id, nombre, puesto, categoria, cuenta, fotografia FROM expedientes WHERE cuenta = 'TMS' ORDER BY nombre ASC`
        : `SELECT id, nombre, puesto, categoria, cuenta, fotografia FROM expedientes ORDER BY nombre ASC`
    );
