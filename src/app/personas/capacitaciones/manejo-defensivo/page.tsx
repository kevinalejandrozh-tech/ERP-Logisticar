...
import {
  PREGUNTAS_MANEJO_DEFENSIVO,
  PREGUNTAS_POR_PAGINA,
  TOTAL_PREGUNTAS,
  TOTAL_PAGINAS,
  type OpcionLetra,
} from "@/lib/manejoDefensivoData";

const NOMBRE_CAPACITACION = "Manejo defensivo";
...
export default function ManejoDefensivoPage() {
  ...
  const correctasHastaAhora = useMemo(
    () => PREGUNTAS_MANEJO_DEFENSIVO.filter((p) => respuestas[p.numero] === p.correcta).length,
    [respuestas]
  );
  const aciertosPorcentaje = Math.round((correctasHastaAhora / TOTAL_PREGUNTAS) * 100);

  const preguntasPagina = PREGUNTAS_MANEJO_DEFENSIVO.slice(pagina * PREGUNTAS_POR_PAGINA, pagina * PREGUNTAS_POR_PAGINA + PREGUNTAS_POR_PAGINA);
  const esUltimaPagina = pagina === TOTAL_PAGINAS - 1;
  const progresoPct = Math.round(((pagina + 1) / TOTAL_PAGINAS) * 100);
  ...
  const finalizarYGuardar = async () => {
    ...
    const respuestasDetalle = PREGUNTAS_MANEJO_DEFENSIVO.map((p) => {
    ...
    const aciertos = Math.round((correctas / TOTAL_PREGUNTAS) * 100);
      ...
      <div className="max-w-[860px] mx-auto px-4 sm:px-6 md:px-8 pt-6 md:pt-10 pb-16">
        ...
        {fase === "intro" && (
          <div className="bg-white rounded-[18px] p-6 md:p-10 shadow-[0_1px_3px_rgba(22,33,92,0.06)] text-center">
            ...
            <h2 className="text-[19px] md:text-[22px] font-bold text-[var(--navy)] mb-2">Evaluación: Manejo defensivo</h2>
            <p className="text-[13px] md:text-[13.5px] text-[var(--gray-400)] max-w-[440px] mx-auto mb-7">
              {TOTAL_PREGUNTAS} preguntas de opción múltiple sobre conductor profesional, actitudes al conducir, señalización y
              técnicas de manejo defensivo. Se mostrarán {PREGUNTAS_POR_PAGINA} preguntas por pantalla hasta cubrir todo el tema.
            </p>
          ...
          <>
            <div className="mb-5">
              <div className="flex items-center justify-between text-[11.5px] text-[var(--gray-400)] mb-1.5">
                ...
                <span>
                  {contestadas} de {TOTAL_PREGUNTAS} respondidas
                </span>
              </div>
          ...
          <div className="bg-white rounded-[18px] p-6 md:p-10 shadow-[0_1px_3px_rgba(22,33,92,0.06)] text-center">
            ...
            <div className="flex items-center justify-center gap-8 mb-7">
              ...
              <div>
                <p className="text-[26px] font-bold text-[var(--navy)] m-0">
                  {correctasHastaAhora}/{TOTAL_PREGUNTAS}
                </p>
            ...
            {contestadas < TOTAL_PREGUNTAS && (
              <p className="text-[12.5px] text-[var(--red)] mb-5">
                Faltan {TOTAL_PREGUNTAS - contestadas} preguntas sin responder. Se guardarán como incorrectas si finalizas ahora.
              </p>
            )}
